  ...
  _detailItemsFor(Section section) {
    ...
    if (section.keyword == 'release') {
      ...
      builder: (context, AsyncSnapshot<List<ReleaseSingle>> snapshot) {
        if (snapshot.hasData) {
          return musicFunctions.buildRelease(snapshot.data);
      ...
      builder: (context, AsyncSnapshot<List<UploadAlbumDetails>> snapshot) {
        if (snapshot.hasData) {
          return musicFunctions.buildAlbum(snapshot.data);
        } else if (snapshot.hasError) {
          return Text(snapshot.error.toString());
        }
        return Center(child: CircularProgressIndicator(
      ...
      builder: (context, AsyncSnapshot<List<Music>> snapshot) {
        if (snapshot.hasData) {
          return musicFunctions.buildSong(snapshot.data);
        } else if (snapshot.hasError) {
          return Text(snapshot.error.toString());
        }
        return Center(child: CircularProgressIndicator(
...
//         leading: SizedBox(width: 32.0, height: 32.0, child: DecoratedBox(
...
//     )
                          ...
                          return _detailItemsFor(section);
                        }).toList(),
                      ),
                    ),
                  ),
