...
class MusicBloc {
  ...
  getSongs() async {
    ...
    String parsed =
        await network.get(Uri.encodeFull(UiData.domain + "/songs/my-music"));
  ...
  getRelease() async {
    ...
    String parsed =
        await network.get(Uri.encodeFull(UiData.domain + "/songs/release"));
    var response = jsonDecode(parsed);
    if (response["status"] == false) {
      snackBar.add('Could not fetch songs');
      return;
  ...
  getAlbum() async {
    ...
    String parsed =
        await network.get(Uri.encodeFull(UiData.domain + "/songs/album"));
        print(parsed);
    var response = jsonDecode(parsed);
    if (response["status"] == false) {
      snackBar.add('Could not fetch songs');
